...
def MakePayment(request,id,amount):
    ...
    if request.user.username in [u['username'] for u in User.objects.all().values('username')]:
        ...
        "WEBSITE": "WEBSTAGING",
        "CALLBACK_URL": 'http://127.0.0.1:8000/dashboard/handlerequest/'+str(request.usernane)+'/'+ str(id)+'/'+str(order_id)+'/' + str(amount) + '/'
    }
...
def handlerequest(request, username, id,order_id, amount):
    ...
    if status == '01':
        ...
        models.BoughtCourses.objects.create(username = username, id = id)
...
def RegisterCard(request):
    ...
    if request.user.username in [u['username'] for u in User.objects.all().values('username')]:
        if request.method == 'POST':
            ...
            card_bank = form.cleaned_data.get('card_bank')
            card_number = form.cleaned_data.get('card_number')
            expiry_month = form.cleaned_data.get('expiry_month')
            expiry_year = form.cleaned_data.get('expiry_year')
            otp = form.cleaned_data.get('otp')
            if(otp == 123456):
                models.CardDetails.objects.create(username = models.User.objects.get(username =request.username), name_on_card = name_on_card, card_bank = card_bank, card_number = card_number, expiry_month = expiry_month, expiry_year = expiry_year)
            else:
                return redirect('fault', fault='OTP NOT VERIFIED')
